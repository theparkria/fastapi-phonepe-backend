...
def send_otp(data: SendOTPRequest):
    # ⚠️ DEV ONLY
    return {"message": f"OTP sent to +91{data.phone}"}

@app.post("/signup")
def signup(data: VerifyOTPRequest):
    exists = supabase.table("users").select("id").eq("phone", data.phone).execute()
    if exists.data:
...
@app.post("/vehicles/add")
def add_vehicle(data: AddVehicleRequest):
    exists = supabase.table("vehicles").select("vehicle_id").eq("car_number", data.car_number).execute()
    if exists.data:
        raise HTTPException(400, "Car already registered")
...
@app.post("/purchase-token")
def purchase_token(data: TokenPurchaseRequest):
    row = supabase.table("purchased_tokens")\
        .select("*")\
        .eq("user_id", data.user_id)\
        .eq("service_type_id", data.service_type_id)\
        .execute()
    ...
    if row.data:
        token = row.data[0]
        new_count = token["token_count"] + data.token_count
        res = supabase.table("purchased_tokens").update({
            "token_count": new_count,
            "updated_at": datetime.utcnow().isoformat()
        }).eq("token_id", token["token_id"]).execute()
    ...
    return {"message": "Tokens purchased", "purchase": res.data[0]}

@app.post("/consume-token")
def consume_token(data: ConsumeTokenRequest):
    rows = supabase.table("purchased_tokens")\
        .select("*")\
        .eq("user_id", data.user_id)\
        .eq("service_type_id", data.service_type_id)\
        .order("created_at")\
...
@app.get("/parking-slots")
def get_parking_slots(unit_id: int = Query(...)):
    slots = supabase.table("parking_slots").select("*").eq("unit_id", unit_id).execute().data
    return {"total_slots": 30, "slots": slots or []}
